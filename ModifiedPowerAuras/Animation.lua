...
function MPOWA:FHide(key)
  ...
  if p and self.frames[key] and self.frames[key][1]:IsVisible() and not self.testall and not p["test"] then
    ...
    elseif p["shrinkanim"] and not self.frames[key][1].translateanim:IsPlaying() and not self.frames[key][1].fadeout:IsPlaying() and
        not self.frames[key][1].rotateanimout:IsPlaying() and not self.frames[key][1].shrink:IsPlaying() then
      ...
      if p["growout"] and not self.frames[key][1].fadout:IsPlaying() and not self.frames[key][1].rotateanimout:IsPlaying() and
          not self.frames[key][1].growout:IsPlaying() then
        self:PlayAnim("fadeout", key, "fadeout")
        self:PlayAnim("rotateanimout", key, "rotateanimout")
      ...
      elseif p["escapeanimout"] and not self.frames[key][1].rotateanimout:IsPlaying() and not self.frames[key][1].escapeanimout:IsPlaying() then
        ...
        if p["fadeout"] and not self.frames[key][1].translateanim:IsPlaying() and not self.frames[key][1].rotateanimout:IsPlaying() and
            not self.frames[key][1].fadeout:IsPlaying() then
          self:PlayAnim("translateanim", key, "translateanim")
          self:PlayAnim("rotateanimout", key, "rotateanimout")
          self:PlayAnim("fadeout", key, "fadeout")
...
function MPOWA:FShow(key)
  ...
  if not self.frames[key][1]:IsVisible() and p["used"] then
    ...
    elseif p["sizeanim"] and not self.frames[key][1].fadein:IsPlaying() and not self.frames[key][1].rotateanimin:IsPlaying() and
        not self.frames[key][1].sizein:IsPlaying() then
      self:PlayAnim("fadein", key, "fadein")
      self:PlayAnim("rotateanimin", key, "rotateanimin")
      self:PlayAnim("sizeanim", key, "sizein")
    else
      if p["growin"] and not self.frames[key][1].fadein:IsPlaying() and not self.frames[key][1].rotateanimin:IsPlaying() and
          not self.frames[key][1].growin:IsPlaying() then
        self:PlayAnim("fadein", key, "fadein")
        self:PlayAnim("rotateanimin", key, "rotateanimin")
