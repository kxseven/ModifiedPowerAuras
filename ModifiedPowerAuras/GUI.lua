...
local timerfont = {UNIT_NAME_FONT, "Fonts\\ARIALN.TTF", "Fonts\\MORPHEUS.TTF", "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\visitor2.TTF",
                   "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\Accidental_Presidency.TTF", "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\Enigma__2.TTF",
                   "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\VeraSe.TTF", "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\AlteHaas.TTF",
                   "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\CaviarDreams.TTF", "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\Expressway.TTF",
                   "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\ExpresswayBold.TTF", "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\Roboto.TTF",
                   "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\The Bad Times.TTF", "Interface\\AddOns\\ModifiedPowerAuras\\fonts\\Vegur.TTF"}

for i = 1, 42 do
  icons["misc2"][locpath .. "Aura" .. i] = true;
end
for i = 43, 84 do
  icons["misc3"][locpath .. "Aura" .. i] = true;
end
for i = 85, 126 do
  icons["misc4"][locpath .. "Aura" .. i] = true;
end
for i = 127, 168 do
  icons["misc5"][locpath .. "Aura" .. i] = true;
end
for i = 169, 210 do
  icons["misc6"][locpath .. "Aura" .. i] = true;
end
for i = 211, 246 do
  icons["misc7"][locpath .. "Aura" .. i] = true;
end
...
function MPOWA:ApplyDynamicGroup(i)
  ...
  if self.frames[i] and val["isdynamicgroup"] then
    ...
    if val["dynamicsorted"] then
      ...
      for cat, va in pairs(self.SAVE) do
        if self.frames[cat] and (tnbr(va["groupnumber"]) == i or cat == i) and (self.active[cat] and not self.NeedUpdate[cat]) or
            (self.NeedUpdate[cat] and not self.active[cat] and (((self:GetCooldown(va["buffname"]) or 0) > 0) or not va["cooldown"]) or va["test"] or
                self.testAll) then
...
function MPOWA:ApplyAttributesToButton(i, button)
  ...
  if not _G("ConfigButton" .. p) or not self.SAVE[i] then
    return
  end
  button:ClearAllPoints()
...
function MPOWA:Edit()
  if ConfigButton1 then
    ...
    if not self.SAVE[self.CurEdit] then
      return
    end
    ...
    MPowa_ConfigFrame_Container_2_Slider_PosX:SetMinMaxValues(MPOWA:GetMinValues(self.SAVE[self.CurEdit].fontoffsetx),
        MPOWA:GetMaxValues(self.SAVE[self.CurEdit].fontoffsetx))
    ...
    MPowa_ConfigFrame_Container_2_Slider_PosY:SetMinMaxValues(MPOWA:GetMinValues(self.SAVE[self.CurEdit].fontoffsety),
        MPOWA:GetMaxValues(self.SAVE[self.CurEdit].fontoffsety))
    ...
    MPowa_ConfigFrame_Container_1_ColorpickerNormalTexture:SetVertexColor(self.SAVE[self.CurEdit].icon_r or 1, self.SAVE[self.CurEdit].icon_g or 1,
        self.SAVE[self.CurEdit].icon_b or 1)
    ...
    MPowa_ConfigFrame_Container_1_Icon_Texture:SetVertexColor(self.SAVE[self.CurEdit].icon_r or 1, self.SAVE[self.CurEdit].icon_g or 1,
        self.SAVE[self.CurEdit].icon_b or 1)
    ...
    MPowa_ConfigFrame_Container_2_2_ColorpickerNormalTexture:SetVertexColor(self.SAVE[self.CurEdit].fontcolor_r, self.SAVE[self.CurEdit].fontcolor_g,
        self.SAVE[self.CurEdit].fontcolor_b)
    ...
    MPowa_ConfigFrame_Container_6_Slider_OrientationText:SetText(MPOWA_SLIDER_DYNAMICORIENTATION ..
                                                                     MPowa_ConfigFrame_Container_6_Slider_Orientation.valuetext[tnbr(
            self.SAVE[self.CurEdit].dynamicorientation)])
    ...
    MPowa_ConfigFrame_Container_1_Slider_BlendModeText:SetText(MPOWA_SLIDER_BLENDMODE ..
                                                                   MPowa_ConfigFrame_Container_1_Slider_BlendMode.valuetext[tnbr(
...
function MPOWA:Checkbutton_USEFONTCOLOR()
  if self.SAVE[self.CurEdit].usefontcolor then
    ...
    _G("TextureFrame" .. self.CurEdit .. "_Timer"):SetTextColor(self.SAVE[self.CurEdit].fontcolor_r, self.SAVE[self.CurEdit].fontcolor_g,
        self.SAVE[self.CurEdit].fontcolor_b, self.SAVE[self.CurEdit].usefontcolor)
  end
end
